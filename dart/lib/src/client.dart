...
class JsontocsvValidationException implements Exception {
  final List<String> errors;

  JsontocsvValidationException(this.errors);

  @override
  String toString() => 'JsontocsvValidationException: ${errors.join("; ")}';
}
...
/// JSON to CSV Converter API client.
///
/// For more information, visit: https://apiverve.com/marketplace/jsontocsv?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [json] (required) - The JSON array to convert to CSV (must be array of objects)
/// * [delimiter] - The delimiter to use in the CSV
/// * [includeHeader] - Whether to include column headers in the output (default: true)
class JsontocsvClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'json': ValidationRule(type: 'array', required: true),
    'delimiter': ValidationRule(type: 'string', required: false),
    'include_header': ValidationRule(type: 'boolean', required: false),
  };

  JsontocsvClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/jsontocsv',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [JsontocsvValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw JsontocsvValidationException(errors);
    }
  }

  /// Execute a request to the JSON to CSV Converter API.
  ///
  /// Parameters are validated before sending the request.
  Future<JsontocsvResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw JsontocsvException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return JsontocsvResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw JsontocsvException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw JsontocsvException('Resource not found');
      } else {
        throw JsontocsvException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is JsontocsvException) rethrow;
      throw JsontocsvException('Request failed: $e');
    }
  }
...
/// Exception thrown by the JSON to CSV Converter API client.
class JsontocsvException implements Exception {
  final String message;

  JsontocsvException(this.message);

  @override
  String toString() => 'JsontocsvException: $message';
}
