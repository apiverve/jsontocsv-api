...
class JsontocsvAPIClientError(Exception):
...
class ValidationError(JsontocsvAPIClientError):
...
class JsontocsvAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"json": {"type": "array", "required": True}, "delimiter": {"type": "string", "required": False, "default": ","}, "include_header": {"type": "boolean", "required": False, "default": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the JsontocsvAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises JsontocsvAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/jsontocsv'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[JsontocsvAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns JSONtoCSVConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises JsontocsvAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise JsontocsvAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise JsontocsvAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise JsontocsvAPIClientError(f"Request failed: {str(e)}")
